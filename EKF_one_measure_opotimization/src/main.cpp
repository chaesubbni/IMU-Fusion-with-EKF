...
double p[7] = { 1.0,1.0, 0.001000, 0.001000 ,0.001000, 1.0 , 1.0 }; // 0.888100 0.999100 0.000100 0.000100 0.000100 0.999100 0.999100
...
0.100000 0.100000 0.001000 0.001000 0.001000 2.700000 3.700000
...
0.100000 0.100000 0.001000 0.001000 0.001000 7.900000 6.200000
...
sensor mean_acc[1000] = {};
sensor mean_gyro[1000] = {};
...
sensor sample_gyro[1500] = {};
sensor sample_acc[1500] = {};

sensor sample_gyro2[1500] = {};
sensor sample_acc2[1500] = {};
...
void setup()
{
    ...
    sum_acc.x = sum_acc.x / 1000;
    sum_acc.y = sum_acc.y / 1000;
    sum_acc.z = sum_acc.z / 1000;

    sum_gyro.x = sum_gyro.x / 1000;
    sum_gyro.y = sum_gyro.y / 1000;
    sum_gyro.z = sum_gyro.z / 1000;
    ...
    var_acc.x = var_acc.x / 999;
    var_acc.y = var_acc.y / 999;
    var_acc.z = var_acc.z / 999;

    var_gyro.x = var_gyro.x / 999;
    var_gyro.y = var_gyro.y / 999;
    var_gyro.z = var_gyro.z / 999;
    ...
    while (sample_cnt < 1500){
      if (IMU.accelerationAvailable() && IMU.gyroscopeAvailable()){
          ...
          ay *= 9.81;
          az *= 9.81;
          sample_acc[sample_cnt] = {ax, ay, az};
          sample_gyro[sample_cnt] = {gx, gy, gz};
          sample_cnt++;            
          delay(WAIT_TIME);
        }
    }
